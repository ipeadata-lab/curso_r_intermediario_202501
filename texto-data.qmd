...
A função `readr::parse_date()` tem o mesmo propósito. A [documentação dessa função é muito útil para entender como formatar datas](https://readr.tidyverse.org/reference/parse_datetime.html#format-specification).
...
Existe uma função do pacote `lubridate` que facilita a conversão de datas: a função `dmy()` (que significa "dia-mês-ano"), mas vamos apresentá-la mais adiante.
